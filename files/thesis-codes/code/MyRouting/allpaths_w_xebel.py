...
class ijPaths:
	...
	def __init__(self, paths, topology, epaths = None):
		self.topology= topology #WEIGHTENING
		if paths:
			#WEIGHTENING:
			# self.Paths = sorted(paths, key=len)
			self.Paths = sorted(paths, key=lambda path: self.topology.sid_path_weight(path))
			self.Weights = [self.topology.sid_path_weight(path) for path in self.Paths]
	...
	def _epath_similarity(self, ep1, ep2):
		...
		# sim = 1.0 * counter / len(shorter) 
		# if len(shorter) == 1:
		# 	log_into(f" *=*=*= similarity {shorter}, {longer} = {sim}", "/tmp/haha")
	...
	def prone(self, acep, aps):
		...
		shortest_len = self.topology.sid_path_weight(self.Paths[0])
		# log_into(f"src {self.src} dst {self.dst} shortest_len = {shortest_len}", "/tmp/haha")
		pronned_paths = []
		pronned_epaths = []
		for i in range(self.PathsNo):
			ep = self.EPaths[i]
			p = self.Paths[i] #WEIGHTENING
			# if len(ep) == 1:
			# 	log_into(f"***edg {ep}", "/tmp/haha")

			if not self._have_similar_epath_in_list(ep, pronned_epaths, acep):
				#WEIGHTENING:
				# if len(ep) <= aps * shortest_len:
				if self.Weights[i] <= aps * shortest_len:
					pronned_epaths.append(ep)
					pronned_paths.append(self.Paths[i])
		#WEIGHTENING
		# return ijPaths(pronned_paths, pronned_epaths)
		return ijPaths(pronned_paths, self.topology, pronned_epaths)
	...
	def print_stats(self, acep, aps):
		...
		# print(" %d>%d ::  ce %.2f  ps %d #paths: %d" % ( self.src , self.dst , round(acep, 2), aps, self.PathsNo))
		print(f" {self.src}>{self.dst} ({round(acep, 2)},{round(aps, 2)}) #paths {self.PathsNo}, sum_lengths: {lens_sum}")
		# print(lens_sum)
		# print(".")
...
class Graph: 
	...
	*   A dictionary with:
	...
	def __init__(self,topology: Topology, topology_name = None, prints = False): 
		dataset = topology.filename
		self.topology = topology
		self.dataset = dataset
		self.topology_name = topology_name or self.topology.name or dataset.split('/')[-1].split('\\')[-1].split('.')[0]
		self.prints = prints
		tbl = file_to_table(dataset)
		array = [[key[0], key[1], self.topology.Links[key].capacity, self.topology.Links[key].weight] for key in self.topology.Links]


		# default dictionary to store graph 
		self.graph = defaultdict(list) 
		self.weights = defaultdict(list) #WEIGHTENING
		#No. of vertices 
		...
		if self.prints:
			print(f"Loaded graph {self.topology_name}, V={self.V}, E={self.E}  from file {self.dataset}:")
			for row in array:
				print(f" {row}")
	...
	def getPaths(self, acep, aps, src, dst):
		...
		if not self._PATHs[pstr][src][dst]:
			...
			ijpaths = ijPaths(paths, self.topology)
	...
	def save(self, dir_to_save = None, filename = None, pstr = None):
		directory = dir_to_save
		if directory == None:
			directory = dataset_pkl_dir(self.dataset)
		fname = filename
		if fname == None:
			fname = self.topology_name + "_PATHs"
		dict_to_save = {}
		if pstr:
	...
	def load(self, dir_to_save = None, filename = None, merge = True, pstr = None):
		...
		if directory == None:
			directory = dataset_pkl_dir(self.dataset)
		fname = filename or self.topology_name + "_PATHs.pkl"
	...
	'''
	def save_paths_from_to(self,  acep, aps, dir_to_save = None, save_at = 50,
						 sources = None, dests = None, dont_load = True, filename = None):
		srcs = sources or range(self.V)
		dsts = dests or range(self.V)
		pstr = self._param_to_str(acep, aps)
		if not self.loaded and not dont_load:
			self.load(dir_to_save = dir_to_save, pstr=pstr, filename=filename)
			self.load(dir_to_save = dir_to_save, pstr=DefaultParams, filename=filename)
		i = 0
			...
			for dst in dsts:
				if self.PATHs(acep, aps, src, dst):
					...
					if i == save_at:
						if self.prints:
							print("\nSaving ...")
						self.save(dir_to_save = dir_to_save, filename=filename, pstr=pstr)
						i = 0
		if self.prints:
			print("\nEND. Saving ...")
		self.save(dir_to_save = dir_to_save, filename=filename)
		if self.prints:
			print("+++ (%s seconds) (N= %d acep= %.2f aps= %.2f) " % (round((time.time() - start_time),2), self.V, acep, aps ))
	def get_paths_from_to(self, acep, aps, sources = None, dests = None,
					    dont_load = False, filename = None, 
						dir_to_save = None, save_at = 50):
		pstr = self._param_to_str(acep, aps)
		self.save_paths_from_to(acep, aps, sources=sources, dests = None, 
						  dont_load = dont_load, 
						  save_at=save_at, 
						  dir_to_save=dir_to_save, 
						  filename=filename)
		pathList = [self.PATHs(acep, aps, src = source) for source in sources]
		return pathList
