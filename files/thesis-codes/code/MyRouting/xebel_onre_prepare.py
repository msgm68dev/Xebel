#!/usr/bin/python3
import os, sys, time, argparse
from utils import *
# Operators = (
#     "add",
#     "multiply",
#     "min"
# )

class equation:
    def __init__(self, dag_node_dict):
        self.left = dag_node_dict
        self.rights = dag_node_dict["Parent_dicts"]
        self.left_key = self.left["key"]
        self.right_keys = [x["key"] for x in self.rights]
        self.left_tuple = self.left["Tuple"]
        self.right_tuples = [x["Tuple"] for x in self.rights]
        self.cost = len(self.rights) - 1

class xonre_prepare:
    def __init__(self, dag_file, log_file, n_workers):
        # Inits:
        start = time.time()
        self.dag_dict = load_pkl_json(dag_file)
        self.dag = self.dag_dict["dag"]
        self.params = "{}-{}".format(self.dag_dict["acep"], self.dag_dict["aps"])
        self.logfile = log_file
        self.n_workers = n_workers
        self.edges_file_to_key = {} # key = edge file, value = edge memcache key
        self.nodes = sorted(self.dag.values(), key=lambda x:len(x["Tuple"]))
        # Checks:
        assert len(self.nodes) == len(set([n["Tuple"] for n in self.nodes]))
        self.logdir = os.path.dirname(log_file)
        os.makedirs(self.logdir, exist_ok=True)
        with open(self.logfile, "a") as f:
            # f.write(f"xoff-3: \n")
            f.close()
        self.logger("xoff-3:", log_date=True)
        # Processes:
        path_counter = edge_counter = mid_counter = 0
        for node in self.nodes:
            src = node["Tuple"][0]
            dst = node["Tuple"][-1]
            node["digest"] = hex(hash(node["Tuple"]))[-8:]
            node["key"] = tuple_to_dashedstr(node["Tuple"])
            if node["is_edge"]:
                edge_counter += 1
                Tuple = node["Tuple"]
            elif node["is_path"]:
                path_counter += 1
            elif node["is_mid_only"]:
                mid_counter += 1
        for node in self.nodes:
            parent_dicts = []
            for parent in node["Parents"]:
                parent_dicts.append(self.dag[parent])
            node["Parent_dicts"] = parent_dicts
        self.Equations = [equation(node) for node in self.nodes if not node["is_edge"] and not node["is_root"]]
        self.Cost = sum(e.cost for e in self.Equations)
        self.Workers = []
        cost_threshold = self.Cost / self.n_workers
        self.sublists = defaultdict(list)
        sublist_idx = 0
        sublist_cost = 0
        for eq in self.Equations:
            if sublist_cost < cost_threshold or sublist_idx == self.n_workers - 1:
                self.sublists[sublist_idx].append(eq)
                sublist_cost += eq.cost
            else:
                sublist_cost = 0
                sublist_idx += 1
                self.sublists[sublist_idx].append(eq)

        # Reports:
        elaps = elapsed_time(start)
        logstr = "### "
        logstr += f"params:{self.params}, "
        logstr += f"dag_file:{dag_file}, "
        logstr += f"#edges:{edge_counter}, "
        logstr += f"#paths:{path_counter}, "
        logstr += f"#mid_onlys:{mid_counter}, "
        logstr += f"dag_process_time:{elaps}, "
        self.logger(logstr)
        # return logstr
    def export_ways(self, ways_file):
        def is_edge(n):
            if n["is_edge"]:
                return 1
            return 0
        def is_path(n):
            if n["is_path"]:
                return 1
            return 0
        def is_mid(n):
            if n["is_mid"]:
                return 1
            return 0
        def is_mid_only(n):
            if n["is_mid_only"]:
                return 1
            return 0
        create_file_with_path(ways_file)
        start = time.time()
        f = open(ways_file, "w+t")
        # key|tuple-dashed|edge?|path?|mid?|midonly?
        # key = <src>_<dst>_<digest>
        f.write("#key|tuple-dashed|edge?|path?|mid?|midonly?\n")
        counter = 0
        for n in self.nodes:
            if n["is_root"]:
                continue
            # if n["is_edge"]:
            #     print("{}".format(n["Tuple"]))
            f.write("{}|{}|{}|{}|{}|{}\n".format(n["key"], tuple_to_dashedstr(n["Tuple"]), 
                                                 is_edge(n), is_path(n), is_mid(n), is_mid_only(n)))
            counter += 1
        f.close()
        logstr = ""
        logstr += f"ways_file:{ways_file}, "
        logstr += f"#nodes:{counter}, "
        logstr += f"write_ways_time:{elapsed_time(start)}, "
        return logstr

        return logstr
    def export_equations(self, equations_file):
        create_file_with_path(equations_file)
        start = time.time()
        f = open(equations_file, "w+t")
        # id|worker|cost|left_key|right_key_1#right_key_2#...
        f.write("#id|worker|cost|left_key|right_key_1#right_key_2#...")
        counter = 0
        for w in range(len(self.sublists)):
            sublist = self.sublists[w]
            for eq in sublist:
                # rkeys = "#".join([r["key"] for r in eq.rights])
                rkeys = "#".join(eq.right_keys)
                f.write("{}|{}|{}|{}|{}\n".format(counter, w, eq.cost, eq.left["key"], rkeys)) 
                counter += 1
        logstr = ""
        logstr += f"equations_file:{equations_file}, "
        logstr += f"#equations:{counter}, "
        logstr += f"#workers:{len(self.sublists)}, "
        logstr += f"write_equations_time:{elapsed_time(start)}, "
        return logstr
    def logger(self, logstr, print_also = True, log_date = False):
        date_str = ""
        if log_date:
            date_str = f"{now_str()} |"
        with open(self.logfile, "a+") as f:
            f.write(f"{date_str} {logstr}\n")
            if print_also:
                print(f"{logstr}")
            f.close()

def prepare_for_onre(CONF = None):
    OFF_CONF = CONF['offline']
    acep = float(OFF_CONF['acep'])
    aps = float(OFF_CONF['aps'])
    params = f"{acep}-{aps}"
    stage2_dir = OFF_CONF['stage2_dir']
    stage3_dir = OFF_CONF['stage3_dir']
    log_file = OFF_CONF['stage3_logfile']
    stage2_dagfile_name = OFF_CONF['stage2_dagfile_name']
    stage3_waysfile_name = OFF_CONF['stage3_waysfile_name']
    stage3_equationsfile_name = OFF_CONF['stage3_equationsfile_name']
    ONRE_CONF = CONF['onre']
    n_workers = int(ONRE_CONF['online_workers'])
    params = f"{acep}-{aps}"
    Dagfile_pkl = os.path.abspath(os.path.join(stage2_dir, params, stage2_dagfile_name))
    Ways_txt = os.path.abspath(os.path.join(stage3_dir, params, stage3_waysfile_name))
    Equations_txt = os.path.abspath(os.path.join(stage3_dir, params, stage3_equationsfile_name))
    X = xonre_prepare(dag_file=Dagfile_pkl, log_file=log_file, n_workers = n_workers)
    log1 = X.export_ways(Ways_txt)
    log2 = X.export_equations(Equations_txt)
    X.logger(f"params: {X.params}, {log1}, {log2}")
    print(f"see:\n {log_file}")
if __name__ == "__main__":
    argv = sys.argv
    argc = len(argv)
    # Define command-line switches:
    parser = argparse.ArgumentParser()
    # Define the --config-file switch
    parser.add_argument("--config-file", help="specify the configuration file")
    parser.add_argument("--stage", help="specify the stage of online phase (default is 0 to run all stages in order)")
    parser.add_argument("args", nargs="*", help="any arguments without switches")
    args = parser.parse_args()
    current_dir = os.getcwd()
    print(f"Here: {current_dir}")
    CONFIG_FILE = args.config_file
    if not CONFIG_FILE:
        if os.path.exists(current_dir + "/xebel.conf"):
            CONFIG_FILE = os.path.join(current_dir, "xebel.conf")
        elif os.path.exists("/root/configs/xebel.conf"):
            CONFIG_FILE = "/root/configs/xebel.conf"
        else:
            print("No config file (xebel.conf) found!")
    print(f"Using config file: {CONFIG_FILE}")
    CONF = load_configs(CONFIG_FILE)
    prepare_for_onre(CONF)