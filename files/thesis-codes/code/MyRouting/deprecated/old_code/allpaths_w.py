...
class ijPaths:
	...
	def __init__(self, paths, topology, epaths = None):
		self.topo= topology #WEIGHTENING
		if paths:
			#WEIGHTENING:
			# self.Paths = sorted(paths, key=len)
			self.Paths = sorted(paths, key=lambda path: self.topo.sid_path_weight(path))
			self.Weights = [self.topo.sid_path_weight(path) for path in self.Paths]
	...
	def prone(self, acep, aps):
		...
		shortest_len = self.topo.sid_path_weight(self.Paths[0])
		...
		return ijPaths(pronned_paths, self.topo, pronned_epaths)
	...
	*   A dictionary with:
	...
	def getPaths(self, acep, aps, src, dst):
		...
		if not self._PATHs[pstr][src][dst]:
			...
			ijpaths = ijPaths(paths, self.topo)
