...
class ShortestForwarding(app_manager.RyuApp):
    ...
    def get_path(self, src, dst):
        ...
        """
        # m.salari: send routing request on socket
        try:
            javab_bytes = self.monitor.outsource.send_routing_request(src, dst)
            javab=javab_bytes[2:-1]
            # self.logger.info(" ***############* 1 Salari route:  {}".format(javab))
            words = str(javab).split(" ")
            src = words[1]
            dst = words[2]
            mypath = words[3].split(',')
            answer=[int(x) for x in mypath]
            self.logger.info("***############* get_path answere (Mostafa Salari): {}".format(answer))
            return answer
        except Exception as e:
            self.logger.info(" ERROR in getting Salari ROUTE: {}".format(str(e)))
            exit(1)
        # \m.salari

        # m.salari: I omit muzixing answere, by commenting the following:
        # shortest_paths = self.awareness.shortest_paths
        # graph = self.awareness.graph
        # answer = shortest_paths.get(src).get(dst)[0]
        # self.logger.info("get_path answere (muzixin. NOT Salari): {}".format(answer))
        # return answer
        # \m.salari
    ...
    def shortest_forwarding(self, msg, eth_type, ip_src, ip_dst):
        ...
        if result:
            ...
            if dst_sw:
                ...
                self.logger.info("xxxxyyyyyzzzz [PATH]%s<-->%s: %s" % (ip_src, ip_dst, path))
