...
class NetworkDelayDetector(app_manager.RyuApp):
        ...
        try:
            for src in self.awareness.graph:
                for dst in self.awareness.graph[src]:
                    ...
                    # m.salari
                    try:
                        javab = self.outsource.send_link_delay(src, dst, delay) 
                        # print("**** delay {}".format(javab))
                    except Exception as e:
                        print(" Error in sending DELAY: {}".format(str(e)))
                    # \m.salari
        except:
            if self.awareness is None:
                self.awareness = lookup_service_brick('awareness')
    ...
    def show_delay_statis(self):
        # m.salari
        self.logger.info("I disabled network_delay_detector.py: NetworkDelayDetector > show_delay_statis")
        return
        # \m.salari
